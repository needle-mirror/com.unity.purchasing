...
| `appleExtensions.GetProductDetails()` | `store.AppleStoreExtendedProductService?.GetProductDetails()` — returns `Dictionary<string, string>`. **NOT removed**, just moved to the product extension service. Basic metadata (title, description, price) is on `product.catalogListings[product.uSku].metadata` (or `cartItem.Product.catalogListings[cartItem.CatalogListingId].metadata` inside a cart context). The legacy `product.metadata` accessor is `[Obsolete]` and forwards to the base listing. |
...
| v4 (Legacy) | v5 (Current) |
|---|---|
| `new SubscriptionManager(product, introJson).getSubscriptionInfo()` | `order.Info.PurchasedProductInfo.FirstOrDefault(p => p.productId == productId)?.subscriptionInfo` — accessed via `IPurchasedProductInfo` on the order's info, NOT on `CartItem`. `CartItem` exposes `Product`, `CatalogListingId`, and `Quantity`. |
...
| `product.metadata.GetAppleProductMetadata()?.isFamilyShareable` | `GetAppleProductMetadata()` extension on `ProductMetadata` is unchanged; access the metadata via `product.catalogListings[product.uSku].metadata.GetAppleProductMetadata()?.isFamilyShareable`. The legacy `product.metadata` accessor is `[Obsolete]` and forwards to the base listing. |
