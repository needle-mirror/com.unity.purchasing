...
{
internal class GoogleCatalogApiFetcher : ICatalogFetcher
{
    const string k_Store = "google";

    readonly IPlatformCatalogApi m_Api;
    readonly IAccessTokens m_TokenProvider;

    public string SecretKey { get; set; } = "purchasing_android";
    ...
    public GoogleCatalogApiFetcher(IPlatformCatalogApi api, IAccessTokens tokenProvider)
    {
    ...
    public async Task<List<ImportedCatalogEntry>> FetchCatalogEntries()
    {
        ...
        if (string.IsNullOrWhiteSpace(packageName))
            throw new InvalidOperationException(
                "Android package name is missing. Configure it in Project Settings > Player.");
        ...
        var headers = new AdminApiHeaders<GoogleCatalogApiFetcher>(accessToken);
