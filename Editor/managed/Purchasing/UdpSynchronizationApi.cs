...
namespace UnityEditor.Purchasing
{
    public static class UdpSynchronizationApi
    {
        ...
        internal static bool CheckUdpAvailability()
        {
            ...
            bool rightVersion = true;
#else
            bool rightVersion = false;
            string unityVersion = Application.unityVersion;
            string[] versionSplit = unityVersion.Split('.');

            if (int.Parse(versionSplit[0]) > 5) // Unity 2017, 2018, etc.
            {
                rightVersion = true;
            }
            else if (int.Parse(versionSplit[0]) == 5) // Unity 5.x
            {
                if (int.Parse(versionSplit[1]) > 6) // Unity 5.7.x.. (do not exist)
                {
                    rightVersion = true;
                }
                else if (int.Parse(versionSplit[1]) == 6) // Unity 5.6.x..
                {
                    rightVersion = !unityVersion.StartsWith("5.6.0");
                }
            }
#endif
            return hasOAuth && rightVersion;
        }
