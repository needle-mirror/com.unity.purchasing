...
3. (optional) Update the `./spec.yaml` with the client api spec.
...
2. Extract and move the generated client to `./Client`:
    ```bash
      rm -rf ./Client && mkdir ./Client && mv ./tmp-client/com.unity.purchasing.webshop/Runtime/* ./Client/ && rm -rf ./tmp-client
    ```
